    ...
    <div className="h-full flex flex-col p-4" dir={isFa ? "rtl" : "ltr"}>
      ...
      <div className="flex items-center justify-between mb-8 px-1">
        <Link href={`/${locale}`} className="flex items-center gap-2 no-underline">
          <div className="w-7 h-7 rounded-lg bg-[var(--fg)] flex items-center justify-center text-[13px] font-black text-[var(--bg)]">R</div>
          <span className="text-sm font-extrabold text-[var(--fg)] tracking-tight">RideX</span>
        </Link>
        <button
          onClick={() => dispatch(setSidebarOpen(false))}
          className="lg:hidden w-7 h-7 rounded-lg bg-[var(--glass)] border border-[var(--bdr)] flex items-center justify-center text-[var(--fg3)] hover:text-[var(--fg)]"
        >
          <X size={14} />
        </button>
      </div>
            ...
            <Link
              ...
              className={cn(
                ...
                isActive
                  ...
                  : "text-[var(--fg3)] hover:text-[var(--fg)] hover:bg-[var(--glass)]"
              )}
            >
              {item.icon}
      ...
      {user && (
        <div className="mt-4 pt-4 border-t border-[var(--bdr)]">
          <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl hover:bg-[var(--glass)] transition-colors cursor-pointer mb-1">
            <Avatar name={user.name} size="sm" />
            <div className="flex-1 min-w-0">
              <p
                className="text-[12.5px] font-semibold text-[var(--fg)] truncate"
                style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
              >
                {user.name}
              </p>
              <p className="text-[10.5px] text-[var(--fg4)] truncate">{user.phone}</p>
            </div>
          </div>
          <button
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed top-0 bottom-0 z-50 w-[260px] bg-[var(--bg2)] border-[var(--bdr)] shadow-2xl lg:hidden"
