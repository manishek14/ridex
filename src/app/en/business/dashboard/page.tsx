// src/app/en/business/dashboard/page.tsx
"use client";
      ...
      <div dir={isFa ? "rtl" : "ltr"}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          {[
            ...
            <Card key={i} hover>
              ...
              <div className="text-[20px] font-black text-fg leading-none mb-1" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
                {s.value}
              </div>
              <p className="text-[11px] text-fg3" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>{s.label}</p>
            </Card>
          ))}
        </div>

        {/* Budget bar */}
        <div className="p-5 rounded-2xl bg-glass border border-bdr mb-6">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-bold text-fg" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
              {isFa ? "بودجه ماهانه" : "Monthly Budget"}
            </p>
            <span className="text-xs text-fg4" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
              {formatPrice(usedBudget, locale)} / {formatPrice(monthlyBudget, locale)}
            </span>
          </div>
          <div className="h-2.5 rounded-full bg-glass2 overflow-hidden">
            <div
          ...
          <div className="flex justify-between mt-1.5">
            <span className="text-[10px] text-fg4">{pct}% {isFa ? "استفاده‌شده" : "used"}</span>
            <span className="text-[10px] text-fg4">{100 - pct}% {isFa ? "باقی‌مانده" : "remaining"}</span>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-glass border border-bdr">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={`/${locale}/business/employees`} className="text-xs text-fg4 hover:text-fg no-underline transition-colors">
              {isFa ? "مدیریت" : "Manage"}
            </Link>
          </div>
              ...
              <tbody>
                {mockEmployees.map((emp) => {
                  ...
                  return (
                    <tr key={emp.id}>
                      <td>
                        <p className="text-[13px] font-semibold text-fg" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>{emp.user.name}</p>
                      </td>
                      <td className="text-[12px] text-fg3" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>{emp.department}</td>
                      <td className="text-[12px]" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>{formatPrice(emp.usedAmount, locale)}</td>
                      <td className="text-[12px] text-fg4" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>{formatPrice(emp.monthlyLimit, locale)}</td>
                      <td>
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-1.5 rounded-full bg-glass2">
                            <div
                              className={`h-full rounded-full ${empPct > 80 ? "bg-red-400" : "bg-green-400"}`}
                              style={{ width: `${empPct}%` }}
                            />
                          </div>
                          <span className="text-[10px] text-fg4 w-8 text-right">{empPct}%</span>
                        </div>
                      </td>
                    </tr>
                  );
