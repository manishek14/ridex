...
import { LifeBuoy, Clock, Filter } from "lucide-react";
    ...
    <DashboardLayout locale={locale} pageTitle={isFa ? "پشتیبانی" : "Support"}>
      <div dir={isFa ? "rtl" : "ltr"} className="flex gap-4 h-[calc(100vh-130px)]">
        ...
        <div className="w-full lg:w-[360px] flex flex-col flex-shrink-0">
          {/* Filter */}
          <div className="flex gap-1 mb-3 p-1 rounded-xl bg-[var(--glass)] border border-[var(--bdr)]">
            {filters.map((f) => (
              <button
                key={f.value}
                onClick={() => setFilter(f.value)}
                className={`flex-1 py-1.5 rounded-lg text-[10.5px] font-semibold transition-all ${
                  filter === f.value ? "bg-[var(--fg)] text-[var(--bg)]" : "text-[var(--fg4)] hover:text-[var(--fg3)]"
                }`}
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[10px] text-[var(--fg4)]">#{ticket.id}</span>
                    <Badge variant={sc.v} className="text-[9px]">{isFa ? sc.fa : sc.en}</Badge>
                  </div>
                  <p className="text-[13px] font-semibold text-[var(--fg)] mb-1" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
                    {pc.icon} {ticket.subject}
                  </p>
                  <p className="text-[11.5px] text-[var(--fg3)] line-clamp-2 mb-2" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
                    {ticket.message}
                  </p>
                  <div className="flex items-center gap-1 text-[var(--fg4)]">
                    <Clock size={10} />
        ...
        <div className="flex-1 hidden lg:flex flex-col">
          {activeTicket ? (
            <div className="p-5 rounded-2xl bg-[var(--glass)] border border-[var(--bdr)] h-full flex flex-col">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <p className="text-[10px] text-[var(--fg4)] mb-1">#{activeTicket.id}</p>
                  <h3 className="text-base font-bold text-[var(--fg)]" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
              ...
              <div className="p-4 rounded-xl bg-[var(--glass2)] border border-[var(--bdr)] flex-1 mb-4">
                <p className="text-sm text-[var(--fg2)] leading-[1.7]" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
                  {activeTicket.message}
                </p>
                <p className="text-[10.5px] text-[var(--fg4)] mt-3">{timeAgo(activeTicket.createdAt, locale)}</p>
              </div>
              ...
              <div className="flex gap-2">
                <textarea
                  ...
                  style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
                />
                <button className="px-4 py-2 rounded-xl bg-[var(--btn-bg)] text-[var(--btn-fg)] text-sm font-bold hover:opacity-88 transition-opacity self-end">
                  {isFa ? "ارسال" : "Send"}
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center">
              <LifeBuoy size={40} className="text-[var(--fg4)] mb-3" />
              <p className="text-sm text-[var(--fg4)]" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
