...
import { mockAdminStats, mockRides, mockTickets, mockRevenueChart } from "@/lib/mock-data";
import { formatPrice, timeAgo, getRideStatusLabel } from "@/lib/utils";
...
// ✅ تعریف نوع برای locale
type Locale = "en" | "fa";

export default function AdminDashboardPage() {
  // ✅ دریافت locale با نوع صحیح
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: stats.todayRides.toLocaleString(),
    ...
    <DashboardLayout locale={locale} pageTitle={isFa ? "داشبورد ادمین" : "Admin Dashboard"}>
      <div dir={isFa ? "rtl" : "ltr"}>
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-6">
          {statCards.map((s, i) => (
            <Card key={i} hover>
              ...
              <div className="text-[22px] font-black text-fg leading-none mb-1">
                {s.value}
              </div>
              <p className="text-[11px] text-fg3">{s.label}</p>
            </Card>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          <div className="lg:col-span-2 p-5 rounded-2xl bg-glass border border-bdr">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-bold text-fg">
                {isFa ? "درآمد ماهانه" : "Monthly Revenue"}
              </p>
              <span className="text-xs text-fg4 px-2 py-1 rounded-lg bg-glass2">
          ...
          <div className="p-5 rounded-2xl bg-glass border border-bdr">
            <p className="text-sm font-bold text-fg mb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="p-5 rounded-2xl bg-glass border border-bdr">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href={`/${locale}/admin/rides`} className="text-xs text-fg4 hover:text-fg no-underline transition-colors">
                {isFa ? "همه" : "View all"}
              </Link>
            </div>
            <div className="flex flex-col gap-2">
              {mockRides.slice(0, 4).map((ride) => {
                ...
                return (
                  <div key={ride.id} className="flex items-center gap-3 py-2 border-b border-bdr last:border-none">
                    <div className="w-8 h-8 rounded-xl bg-glass2 flex items-center justify-center text-sm flex-shrink-0">🚗</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12.5px] font-semibold text-fg truncate">
                        {ride.destination?.address || "مقصد"}
                      </p>
                      <p className="text-[10.5px] text-fg4">{timeAgo(ride.createdAt, locale)}</p>
                    </div>
                    <div className="text-right">
                      ...
                      <Badge variant={ride.status === "completed" ? "green" : ride.status === "cancelled" ? "red" : "blue"} className="text-[9px]">
                        {st?.[locale === "fa" ? "fa" : "en"] || ride.status}
                      </Badge>
                    </div>
                  </div>
                );
          ...
          <div className="p-5 rounded-2xl bg-glass border border-bdr">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href={`/${locale}/admin/support`} className="text-xs text-fg4 hover:text-fg no-underline transition-colors">
                {isFa ? "همه" : "View all"}
              </Link>
            </div>
            <div className="flex flex-col gap-3">
              {mockTickets.map((ticket) => {
                ...
                return (
                  <div key={ticket.id} className="flex items-start gap-2.5 py-2.5 border-b border-bdr last:border-none">
                    <div className="w-8 h-8 rounded-xl bg-glass2 flex items-center justify-center text-sm flex-shrink-0">
                      <LifeBuoy size={14} className="text-fg3" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12.5px] font-semibold text-fg truncate">
                        {pMap[ticket.priority]} {ticket.subject}
                      </p>
                      <p className="text-[10.5px] text-fg4">{timeAgo(ticket.createdAt, locale)}</p>
                    </div>
