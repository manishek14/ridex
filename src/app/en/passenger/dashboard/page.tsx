  ...
  return (
    <DashboardLayout locale={locale} pageTitle={isFa ? "داشبورد" : "Dashboard"}>
      ...
      <div className="mb-6" dir={isFa ? "rtl" : "ltr"}>
        ...
        <p
          className="text-sm text-[var(--fg3)] mt-0.5"
          style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
        >
      ...
      {/* Quick Book */}
      <div
        className="mb-6 p-5 rounded-2xl bg-[var(--fg)] relative overflow-hidden"
        dir={isFa ? "rtl" : "ltr"}
      >
        ...
        <div className="relative z-10 flex items-center justify-between gap-4">
          <div>
            <p
              className="text-sm font-bold text-[var(--bg)] mb-1"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {isFa ? "رزرو سریع سفر" : "Quick Book a Ride"}
            </p>
            <p
              className="text-xs text-[var(--bg)]/60"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
          ...
          <Link
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[var(--bg)] text-[var(--fg)] text-sm font-bold no-underline hover:opacity-88 transition-opacity"
            style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
          >
            <MapPin size={14} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6" dir={isFa ? "rtl" : "ltr"}>
        {stats.map((s, i) => (
          <Card key={i} hover className="flex flex-col gap-2">
            ...
            <div
              className="text-[22px] font-black text-[var(--fg)] leading-none"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {s.value}
            </div>
            <div style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
              <p className="text-[11px] font-semibold text-[var(--fg2)]">{s.label}</p>
              <p className="text-[10px] text-[var(--fg4)]">{s.sub}</p>
            </div>
          </Card>
        ))}
      </div>
      ...
      <div dir={isFa ? "rtl" : "ltr"}>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link
            ...
            className="flex items-center gap-1 text-xs text-[var(--fg3)] hover:text-[var(--fg)] no-underline transition-colors"
            style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
          >
            {isFa ? "همه" : "View all"}
          ...
          {rides.slice(0, 4).map((ride) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-3 p-3.5 rounded-xl bg-[var(--glass)] border border-[var(--bdr)] hover:bg-[var(--glass2)] hover:border-[var(--bdr2)] transition-all duration-150 no-underline group"
              >
                <div className="w-9 h-9 rounded-xl bg-[var(--glass2)] flex items-center justify-center text-base flex-shrink-0">
                  🚗
                </div>
                <div className="flex-1 min-w-0">
                  <p
                    className="text-[13px] font-semibold text-[var(--fg)] truncate"
                    style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
                  >
                    {ride.destination.address}
                  </p>
                  <p className="text-[11px] text-[var(--fg4)] flex items-center gap-1 mt-0.5">
                    <Clock size={10} />
