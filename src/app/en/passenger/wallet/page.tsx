...
export default function WalletPage() {
  ...
  const handleDeposit = async () => {
    ...
    toast.success(isFa ? `${formatPrice(amount)} به کیف پول اضافه شد` : `${formatPrice(amount, "en")} added to wallet`);
    setDepositAmount("");
      ...
      <div className="max-w-xl" dir={isFa ? "rtl" : "ltr"}>
        ...
        <div className="wallet-card mb-5 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <p
              className="text-xs font-bold text-[var(--bg)]/60 mb-2 tracking-wide"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {isFa ? "موجودی کیف پول" : "WALLET BALANCE"}
            </p>
            <div
              className="text-[36px] font-black text-[var(--bg)] leading-none mb-4"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-black/10 text-[var(--bg)] text-xs font-bold hover:bg-black/20 transition-colors"
                style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
              >
                <Plus size={12} />
        ...
        {showDeposit && (
          <motion.div
            ...
            className="p-5 rounded-2xl bg-[var(--glass)] border border-[var(--bdr)] mb-5"
          >
            <p
              className="text-sm font-bold text-[var(--fg)] mb-3"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                    depositAmount === String(a)
                      ...
                      : "bg-[var(--glass)] text-[var(--fg3)] border-[var(--bdr)] hover:border-[var(--bdr2)]"
                  }`}
                  style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
                >
          ...
          <div className="flex flex-col gap-2">
            {wallet?.transactions.map((tx) => {
              ...
              return (
                <div
                  key={tx.id}
                  className="flex items-center gap-3 p-3.5 rounded-xl bg-[var(--glass)] border border-[var(--bdr)]"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-[var(--fg4)]">{formatDate(tx.createdAt, locale)}</p>
                  </div>
