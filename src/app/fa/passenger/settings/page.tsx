...
function SettingRow({ icon, label, sub, action, danger, href, onClick }: SettingRowProps) {
  const content = (
    <div className={cn(
      ...
      danger ? "hover:bg-red-500/5" : "hover:bg-[var(--glass)]",
      (href || onClick) && "cursor-pointer"
    )}
      onClick={onClick}
    >
      <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0",
        danger ? "bg-red-500/10 text-red-400" : "bg-[var(--glass2)] text-[var(--fg3)]"
      )}>
        {icon}
      </div>
      <div className="flex-1">
        <p className={cn("text-sm font-semibold", danger ? "text-red-400" : "text-[var(--fg)]")}>{label}</p>
        {sub && <p className="text-xs text-[var(--fg4)] mt-0.5">{sub}</p>}
      </div>
      {action ?? (href ? <ChevronLeft size={14} className="text-[var(--fg4)]" /> : null)}
    </div>
  );
  ...
  return (
    <DashboardLayout locale={locale} pageTitle={isFa ? "تنظیمات" : "Settings"}>
      <div className="max-w-lg" dir={isFa ? "rtl" : "ltr"}>
        {/* Appearance */}
        <div className="mb-4">
          <p className="text-xs font-bold text-[var(--fg4)] tracking-widest mb-2 px-1" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
              ...
              action={
                <button
                  ...
                  className={cn(
                    ...
                    resolved === "dark" ? "bg-[var(--fg)] border-[var(--fg)]" : "bg-[var(--glass2)] border-[var(--bdr2)]"
                  )}
                >
                  <span className={cn(
                    "absolute top-0.5 w-5 h-5 rounded-full bg-[var(--bg)] transition-all duration-300",
                  ...
                  {[{ code: "fa", label: "فارسی" }, { code: "en", label: "EN" }].map((lang) => (
                    <Link key={lang.code} href={`/${lang.code}`}
                      className={cn("px-2.5 py-1 rounded-lg text-xs font-semibold no-underline transition-all",
                        locale === lang.code ? "bg-[var(--fg)] text-[var(--bg)]" : "bg-[var(--glass2)] text-[var(--fg3)] hover:text-[var(--fg)]"
                      )}>
                      {lang.label}
                    </Link>
                  ))}
        ...
        <div className="mb-4">
          ...
          <div className="rounded-2xl bg-[var(--glass)] border border-[var(--bdr)] overflow-hidden divide-y divide-[var(--bdr)]">
            {[
              ...
              <SettingRow
                ...
                sub={item.sub}
                action={
                  <div className="w-11 h-6 rounded-full bg-[var(--fg)] border border-[var(--fg)] relative">
                    <span className="absolute top-0.5 left-[22px] w-5 h-5 rounded-full bg-[var(--bg)]" />
                  </div>
                }
              />
            ))}
