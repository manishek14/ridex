...
const typeIcon: Record<string, React.ReactNode> = {
  ...
  system: <Info   size={15} className="text-fg3" />,
};
...
export default function NotificationsPage() {
  ...
  return (
    <DashboardLayout locale={locale} pageTitle={isFa ? "اعلان‌ها" : "Notifications"}>
      <div className="max-w-xl mx-auto text-center" dir={isFa ? "rtl" : "ltr"}>
        {/* Header row */}
        <div className="flex items-center justify-between mb-5 text-start">
          <p className="text-xs text-fg4" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
            {unreadCount > 0
        ...
        {items.length === 0 ? (
          <div className="text-center py-16">
            <Bell size={36} className="text-fg4 mx-auto mb-3" />
            <p className="text-sm text-fg3" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
          ...
          <AnimatePresence>
            <div className="flex flex-col gap-2">
              {items.map((n) => (
                ...
                >
                  <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0", typeBg[n.type] ?? "bg-glass2")}>
                    {typeIcon[n.type]}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12px] text-fg3 mt-0.5 leading-[1.5]" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
                      {n.message}
                    </p>
                    <p className="text-[10.5px] text-fg4 mt-1.5">
