    ...
    <div className="min-h-screen bg-[var(--bg)] flex flex-col">
      ...
      <div className="flex-1 px-4 py-6 max-w-4xl mx-auto w-full space-y-6">
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-[var(--fg)] to-[var(--fg2)] rounded-2xl p-6 text-[var(--bg)] relative overflow-hidden"
        >
          ...
          <div className="relative z-10">
            ...
            <div className="flex gap-3 mt-4">
              <Button className="flex-1 bg-[var(--bg)] text-[var(--fg)] hover:opacity-80">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[var(--glass)] border border-[var(--bdr)] rounded-xl p-4 text-center">
            <p className="text-xs text-[var(--fg4)]">کل واریز</p>
            <p className="text-xl font-black text-green-400">{totalIncome.toLocaleString()}</p>
          </div>
          <div className="bg-[var(--glass)] border border-[var(--bdr)] rounded-xl p-4 text-center">
            <p className="text-xs text-[var(--fg4)]">کل برداشت</p>
            <p className="text-xl font-black text-red-400">{totalExpense.toLocaleString()}</p>
          </div>
        </div>

        {/* Transactions */}
        <div className="bg-[var(--bg2)] border border-[var(--bdr)] rounded-2xl p-5">
          <h3 className="text-sm font-bold text-[var(--fg)] mb-4">تاریخچه تراکنش‌ها</h3>
            ...
            {transactions.map((tx, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center justify-between p-3 rounded-xl bg-[var(--glass)] border border-[var(--bdr)]"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-[var(--fg)]">{tx.description}</p>
                      <div className="flex items-center gap-2 text-xs text-[var(--fg4)]">
                        <span>{tx.date}</span>
                        <span>•</span>
