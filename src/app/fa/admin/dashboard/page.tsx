...
import { ShieldCheck, Map, AlertCircle } from "lucide-react";
  ...
  return (
    <DashboardLayout locale={locale} pageTitle={isFa ? "پنل مدیریت رایدکس" : "RideX Admin Panel"}>
      <div className="max-w-[1400px] mx-auto space-y-10 pb-10" dir={isFa ? "rtl" : "ltr"}>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-8 rounded-[32px] bg-fg text-bg relative overflow-hidden">
          ...
          <div className="flex items-center gap-6 relative z-10">
            <div className="w-16 h-16 rounded-2xl bg-bg text-fg flex items-center justify-center shadow-2xl">
              <ShieldCheck size={32} />
            </div>
            <div>
          ...
          <div className="flex items-center gap-4 relative z-10">
            <div className="flex -space-x-3 rtl:space-x-reverse">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-10 h-10 rounded-full border-2 border-fg bg-bg2 flex items-center justify-center text-[10px] font-bold text-fg">
                  {i}
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
            <section className="p-8 rounded-[32px] bg-bg2 border border-bdr">
            ...
            <section className="p-8 rounded-[32px] bg-bg2 border border-bdr">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-black text-fg" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-glass border border-bdr">
                    <div className="flex items-center gap-4">
                      <div className={`w-2 h-2 rounded-full ${act.type === "ride" ? "bg-green-500" : act.type === "user" ? "bg-blue-500" : "bg-orange-500"}`} />
                      <p className="text-sm font-bold text-fg">
                        <span className="text-fg4 ml-1">{act.user}</span> {act.action}
                      </p>
                    </div>
                    <span className="text-[10px] font-bold text-fg4">{act.time}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <div className="p-8 rounded-[32px] bg-red-500/5 border border-red-500/10">
              ...
              <div className="space-y-4">
                <div className="p-4 rounded-2xl bg-white/5 border border-red-500/20">
                  <p className="text-xs font-bold text-fg mb-1">{isFa ? "ترافیک بالا در منطقه ۱" : "High Traffic in District 1"}</p>
                  <p className="text-[10px] text-fg4">{isFa ? "کمبود راننده در این منطقه گزارش شده است." : "Driver shortage reported in this area."}</p>
                </div>
                <div className="p-4 rounded-2xl bg-white/5 border border-red-500/20">
                  <p className="text-xs font-bold text-fg mb-1">{isFa ? "تأخیر در درگاه پرداخت" : "Payment Gateway Delay"}</p>
                  <p className="text-[10px] text-fg4">{isFa ? "زمان پاسخگویی درگاه بانک ملت افزایش یافته است." : "Mellat Bank response time is high."}</p>
                </div>
              </div>
            </div>

            <div className="p-8 rounded-[32px] bg-glass border border-bdr">
              <h3 className="text-xs font-black text-fg4 uppercase tracking-widest mb-6">{isFa ? "عملکرد کلی" : "Overall Performance"}</h3>
