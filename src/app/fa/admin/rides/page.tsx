  ...
  return (
    <DashboardLayout locale={locale} pageTitle={isFa ? "مدیریت سفرها" : "Rides"}>
      <div dir={isFa ? "rtl" : "ltr"}>
        {/* Search */}
        <div className="flex items-center gap-2 mb-5 px-3.5 py-2.5 rounded-xl bg-glass border border-bdr focus-within:border-bdr2 w-full max-w-sm">
            ...
            <table className="data-table">
              ...
              <tbody>
                {filtered.map((ride) => {
                  ...
                  return (
                    <tr key={ride.id}>
                      <td className="font-mono text-[11px] text-fg4">{ride.id}</td>
                      <td>
                      ...
                      <td className="text-[11px] text-fg4">{formatDate(ride.createdAt, locale)}</td>
                    </tr>
                  );
                })}
              </tbody>
