  ...
  return (
    <DashboardLayout locale={locale} pageTitle={isFa ? "پنل سازمانی" : "Business Panel"}>
      <div className="w-full px-4 md:px-6 py-4 md:py-6 space-y-6" dir={isFa ? "rtl" : "ltr"}>
        ...
        {/* Header Section */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-5 md:p-6 rounded-2xl bg-bg2 border border-bdr relative overflow-hidden">
          <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-r from-fg/5 to-transparent pointer-events-none" />
          
          <div className="flex items-center gap-4 relative z-10 w-full md:w-auto">
            <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-fg text-bg flex items-center justify-center shadow-2xl flex-shrink-0">
              <Building2 size={24} className="md:hidden" />
              <Building2 size={28} className="hidden md:block" />
            </div>
            <div>
              <h2 className="text-lg md:text-xl font-black text-fg">
                شرکت فناوری نوین
              </h2>
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-2 py-0.5 rounded-lg bg-glass border border-bdr text-[10px] font-bold text-fg4">
                  شناسه: ۸۴۹۲۰
                </span>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 relative z-10 w-full sm:w-auto">
            <div className="text-center sm:text-left">
              <p className="text-[10px] font-black text-fg4 uppercase tracking-widest">
                اعتبار سازمانی
              </p>
              <p className="text-lg md:text-xl font-black text-fg">
                ۱۲,۵۰۰,۰۰۰
                <span className="text-xs font-bold mr-1 opacity-60">تومان</span>
              </p>
            </div>
            <Link href={`/${locale}/wallet`} className="w-full sm:w-auto no-underline">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-3 p-4 md:p-5 rounded-2xl bg-bg2 border border-bdr">
            <EmployeeList locale={locale} />
          </div>

          <div className="space-y-4">
            <div className="p-5 rounded-2xl bg-fg text-bg">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-4 rounded-2xl bg-glass border border-bdr text-center">
                <Calendar size={16} className="mx-auto text-fg3 mb-1" />
                <p className="text-lg font-black text-fg">۱۴ روز</p>
                <p className="text-[9px] text-fg4">روز کاری</p>
              </div>
              <div className="p-4 rounded-2xl bg-glass border border-bdr text-center">
                <TrendingUp size={16} className="mx-auto text-green-400 mb-1" />
                <p className="text-lg font-black text-fg">+۲۳٪</p>
                <p className="text-[9px] text-fg4">رشد ماهانه</p>
              </div>
            </div>
