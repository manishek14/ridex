    ...
    <div className="min-h-screen bg-[var(--bg)] flex flex-col">
      ...
      <div className="flex-1 px-4 py-6 max-w-4xl mx-auto w-full space-y-8">
        ...
        >
          ...
          <h1 className="text-3xl sm:text-4xl font-black text-[var(--fg)]">تماس با ما</h1>
        ...
        >
          {contactInfo.map((info, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-[var(--glass)] border border-[var(--bdr)] rounded-xl p-4 flex items-start gap-3 hover:bg-[var(--glass2)] transition-all"
              >
                <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center flex-shrink-0">
                  <Icon size={18} className="text-blue-400" />
                </div>
                <div>
                  <p className="text-xs text-[var(--fg4)]">{info.label}</p>
                  <p className="text-sm font-semibold text-[var(--fg)]">{info.value}</p>
                  <p className="text-xs text-[var(--fg3)]">{info.sub}</p>
                </div>
              </div>
            );
          })}
        ...
        <motion.div
          ...
          className="bg-[var(--bg2)] border border-[var(--bdr)] rounded-2xl p-6"
        >
          <h2 className="text-xl font-bold text-[var(--fg)] mb-4 text-center">
            ارسال پیام
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            />
            <div>
              <label className="text-xs font-medium text-[var(--fg3)] block mb-1">
                متن پیام
              </label>
              <textarea
                className="w-full px-4 py-3 rounded-xl bg-[var(--glass)] border border-[var(--bdr)] text-[var(--fg)] text-sm placeholder:text-[var(--fg4)] outline-none focus:border-[var(--bdr2)] transition-all min-h-[120px] resize-none"
        ...
        >
          <p className="text-sm text-[var(--fg3)]">ما را در شبکه‌های اجتماعی دنبال کنید</p>
