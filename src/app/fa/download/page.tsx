  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <div className="flex-1 px-4 py-6 max-w-3xl mx-auto w-full space-y-8">
        ...
        >
          <div className="bg-bg2 border border-bdr rounded-2xl p-6 text-center">
            <div className="w-40 h-40 mx-auto bg-glass rounded-xl border border-bdr flex items-center justify-center">
              <div className="text-center">
                <div className="text-6xl mb-2">📱</div>
                <p className="text-xs text-fg4">اسکن کنید</p>
              </div>
            </div>
        ...
        >
          <Button className="w-full py-6 text-base gap-3 rounded-2xl bg-fg text-bg hover:opacity-90">
            <Apple size={24} />
        ...
        <motion.div
          ...
          className="bg-glass border border-bdr rounded-2xl p-4 flex items-center gap-3"
        >
          <div className="flex-1 bg-bg2 rounded-xl px-4 py-3 text-sm text-fg3 truncate font-mono">
            ridex.ir/download
          </div>
          <button
            onClick={copyLink}
            className="px-4 py-3 rounded-xl bg-fg text-bg font-bold text-sm hover:opacity-90 transition-all whitespace-nowrap"
          >
        ...
        >
          {features.map((feature, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-glass border border-bdr rounded-xl p-4 text-center hover:bg-glass2 transition-all"
              >
                <Icon size={20} className="mx-auto text-fg3 mb-2" />
                <span className="text-xs text-fg3">{feature.label}</span>
              </div>
            );
          })}
        </motion.div>
        ...
        {/* Footer */}
        <p className="text-center text-xs text-fg4 pt-4 border-t border-bdr">
