      ...
      <div className="grid md:grid-cols-2 gap-14 items-center">
        ...
        <div>
          <div className="rv d1">
            ...
            <h2
              className="text-[clamp(28px,3.8vw,48px)] font-black tracking-tight leading-[1.05] mb-4 text-[var(--fg)]"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {isFa ? "هوشمند، سریع، دقیق" : "Intelligent, Fast, Precise"}
            </h2>
            <p
              className="text-[15px] text-[var(--fg3)] mb-8 leading-[1.75]"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {isFa
          ...
          <div className="rv d2 grid grid-cols-2 gap-3">
            {aiFeatures.map((f, i) => {
              ...
              return (
                <div
                  key={i}
                  className="p-4 rounded-xl bg-[var(--glass)] border border-[var(--bdr)] hover:bg-[var(--glass2)] hover:border-[var(--bdr2)] transition-all duration-200"
                >
                  <div className="w-9 h-9 rounded-lg bg-[var(--glass2)] flex items-center justify-center text-base mb-3">
                    {f.icon}
                  </div>
                  <h4
                    className="text-[13px] font-bold text-[var(--fg)] mb-1"
                    style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
                  >
                    {d.title}
                  </h4>
                  <p
                    className="text-[11.5px] text-[var(--fg3)] leading-[1.6]"
                    style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
                  >
                    {d.desc}
                  </p>
        ...
        {/* Right - Chat UI */}
        <div className="rv d2">
          <div className="rounded-2xl bg-[var(--bg2)] border border-[var(--bdr)] overflow-hidden">
            {/* Chat header */}
            <div className="px-4 py-3 border-b border-[var(--bdr)] flex items-center gap-3">
              <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
              <span
                className="text-xs font-semibold text-[var(--fg3)]"
                style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
              >
            ...
            <div className="p-4 space-y-3 min-h-[240px]" dir={isFa ? "rtl" : "ltr"}>
                ...
                >
                  <div
                    className={`max-w-[80%] px-3.5 py-2 rounded-xl text-[12.5px] leading-[1.6] ${
                      msg.role === "user"
                        ? "bg-[var(--glass2)] border border-[var(--bdr)] text-[var(--fg2)]"
              ...
              {visibleMessages < chatMessages.length && visibleMessages > 0 && visibleMessages % 2 === 1 && (
                <div className={`flex ${isFa ? "justify-end" : "justify-start"}`}>
                  <div className="flex gap-1 px-3.5 py-2 rounded-xl bg-[var(--fg)]/10 border border-[var(--bdr)]">
                    {[0, 1, 2].map((j) => (
                      <span
                        key={j}
                        className="w-1.5 h-1.5 rounded-full bg-[var(--fg3)] animate-[pulse2_1.2s_ease-in-out_infinite]"
            ...
            {/* Input */}
            <div className="px-4 py-3 border-t border-[var(--bdr)] flex gap-2">
              <input
                className="flex-1 py-2 px-3.5 rounded-lg text-xs bg-[var(--glass)] border border-[var(--bdr)] text-[var(--fg)] placeholder:text-[var(--fg4)] outline-none"
                placeholder={isFa ? "مقصد خود را بنویسید..." : "Type your destination..."}
                style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
                readOnly
              />
              <button className="px-3 py-2 rounded-lg bg-[var(--btn-bg)] text-[var(--btn-fg)] text-xs font-bold">
                {isFa ? "ارسال" : "Send"}
              </button>
            </div>
          </div>
