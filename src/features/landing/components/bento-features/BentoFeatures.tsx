  ...
  return (
    ...
    >
      ...
      <div className="rv d1 mb-12 text-center">
        <p className="text-[10.5px] font-bold tracking-[3px] uppercase text-[var(--fg4)] mb-3">{t.tag}</p>
        <h2
        ...
        <p
          className="text-[15px] text-[var(--fg3)] max-w-[480px] mx-auto"
          style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
        >
          {t.subtitle}
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Big map card */}
        <div className="rv d1 md:col-span-2 rounded-2xl bg-[var(--glass)] border border-[var(--bdr)] p-6 min-h-[220px] relative overflow-hidden group hover:border-[var(--bdr2)] transition-all duration-300">
          <div className="relative z-10">
            ...
            <h3
              className="text-xl font-extrabold text-[var(--fg)] mb-2"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {isFa ? "نقشه هوشمند" : "Smart Map"}
            </h3>
            <p
            ...
            >
              {isFa
                ? "یکپارچه با نشان — دقیق‌ترین نقشه ایران"
                : "Integrated with Neshan — Iran's most accurate map"}
            </p>
          </div>
          {/* Decorative map grid */}
          <div
          ...
          {[
            ...
            <div
              key={i}
              className="absolute w-6 h-6 rounded-full bg-[var(--bg2)] border border-[var(--bdr2)] flex items-center justify-center text-xs animate-[float_5s_ease-in-out_infinite]"
        ...
        {/* Wallet card */}
        <div className="rv d2 rounded-2xl bg-[var(--fg)] p-6 relative overflow-hidden group min-h-[220px]">
          <div className="relative z-10">
            <div className="text-2xl mb-3">💳</div>
            <h3
              className="text-lg font-extrabold text-[var(--bg)] mb-2"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {isFa ? "کیف پول" : "Wallet"}
            </h3>
            <div
              className="text-3xl font-black text-[var(--bg)] mb-1"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {isFa ? "۱,۲۳۳,۰۰۰" : "1,233,000"}
            </div>
            <div className="text-sm text-[var(--bg)]/60">
              {isFa ? "تومان موجودی" : "T balance"}
            </div>
          </div>
        ...
        {t.items.map((item, i) => (
          <div
            key={i}
            className={`rv d${Math.min(i + 1, 4)} rounded-2xl bg-[var(--glass)] border border-[var(--bdr)] p-5 hover:bg-[var(--glass2)] hover:border-[var(--bdr2)] hover:-translate-y-0.5 transition-all duration-200`}
          >
            <div className="w-10 h-10 rounded-xl bg-[var(--glass2)] flex items-center justify-center text-lg mb-3">
              {item.icon}
            </div>
            <h4
              className="text-[14px] font-bold text-[var(--fg)] mb-1.5"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {item.title}
            </h4>
            <p
              className="text-[12px] text-[var(--fg3)] leading-[1.65]"
              style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}
            >
              {item.desc}
            </p>
