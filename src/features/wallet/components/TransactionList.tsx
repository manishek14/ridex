  ...
  return (
    <div className="space-y-4" dir={isFa ? "rtl" : "ltr"}>
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="text-xs font-bold text-fg4 hover:text-fg transition-colors">
      ...
      <div className="space-y-2">
        {mockTransactions.map((tx, i) => (
          <motion.div
            ...
            className="flex items-center justify-between p-4 rounded-2xl bg-bg2 border border-bdr hover:border-bdr2 transition-all group"
          >
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-glass border border-bdr flex items-center justify-center group-hover:scale-110 transition-transform">
                {getIcon(tx.type)}
              </div>
              <div>
                ...
                <p className="text-[10px] text-fg4 font-medium">{tx.date}</p>
              </div>
            </div>
            <div className="text-left">
              <p className={`text-sm font-black ${tx.amount > 0 ? "text-green-500" : "text-fg"}`} style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
                {tx.amount > 0 ? "+" : ""}{new Intl.NumberFormat(isFa ? "fa-IR" : "en-US").format(tx.amount)}
              </p>
              <p className="text-[9px] font-bold text-fg4 uppercase">
                {isFa ? "تومان" : "IRT"}
              </p>
            </div>
          </motion.div>
