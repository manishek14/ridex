    ...
    <div className="space-y-6" dir={isFa ? "rtl" : "ltr"}>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <div className="relative flex-1 md:w-64">
            <Search className="absolute right-3 top-1/2 -translate-y-1/2 text-fg4" size={16} />
            <input 
              type="text" 
              placeholder={isFa ? "جستجوی کارمند..." : "Search employee..."}
              className="w-full pr-10 pl-4 py-2 rounded-xl bg-bg2 border border-bdr text-xs outline-none focus:border-fg transition-all"
            />
          </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          <thead>
            <tr className="text-[10px] font-black text-fg4 uppercase tracking-widest border-b border-bdr">
          ...
          <tbody className="divide-y divide-bdr">
            {employees.map((emp, i) => (
              <motion.tr 
                ...
                className="group hover:bg-glass transition-colors"
              >
                <td className="py-4 pr-4">
                  <div className="flex items-center gap-3">
                    <Avatar name={emp.name} size="sm" />
                    <span className="text-sm font-bold text-fg">{emp.name}</span>
                  </div>
                </td>
                <td className="py-4 text-xs text-fg3 font-medium">{emp.role}</td>
                <td className="py-4 text-sm font-black text-fg">
                  {new Intl.NumberFormat(isFa ? "fa-IR" : "en-US").format(emp.credit)}
                  <span className="text-[9px] mr-1 opacity-50">{isFa ? "تومان" : "IRT"}</span>
                </td>
                <td className="py-4 text-sm font-bold text-fg2">{emp.rides}</td>
                <td className="py-4">
