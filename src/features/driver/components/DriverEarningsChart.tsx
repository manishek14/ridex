...
export function DriverEarningsChart({ locale = "fa" }: { locale?: string }) {
  ...
  return (
    <div className="p-6 rounded-[24px] bg-[var(--bg2)] border border-[var(--bdr)] h-[400px] flex flex-col" dir={isFa ? "rtl" : "ltr"}>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-[var(--fg4)]">{isFa ? "مجموع درآمد ۷ روز اخیر" : "Total income for last 7 days"}</p>
        </div>
        <div className="px-3 py-1.5 rounded-lg bg-[var(--glass)] border border-[var(--bdr)] text-[11px] font-bold text-[var(--fg2)]">
          {isFa ? "تومان" : "IRT"}
        </div>
      </div>
