...
export function DriverProfile({ locale = "fa" }: DriverProfileProps) {
  ...
  const onSubmit = async (_data: DriverProfileFormData) => {
    setIsSaving(true);
    try {
      await new Promise((resolve) => setTimeout(resolve, 1000));
      // Show a browser alert as requested + reset the form fields.
      ...
      toast.success(isFa ? "اطلاعات با موفقیت ثبت شد" : "Information saved");
      reset({
        name: "",
        email: "",
        phone: "",
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-glass border border-bdr rounded-xl p-3 text-center">
          <div className="flex items-center justify-center gap-1 text-yellow-400">
            <Star size={14} />
            <span className="text-sm font-bold text-fg">4.92</span>
          </div>
          <p className="text-[10px] text-fg4">{isFa ? "امتیاز" : "Rating"}</p>
        </div>
        <div className="bg-glass border border-bdr rounded-xl p-3 text-center">
          <div className="flex items-center justify-center gap-1 text-blue-400">
            <Car size={14} />
            <span className="text-sm font-bold text-fg">1,842</span>
          </div>
          <p className="text-[10px] text-fg4">{isFa ? "سفر" : "Rides"}</p>
        </div>
        <div className="bg-glass border border-bdr rounded-xl p-3 text-center">
          <div className="flex items-center justify-center gap-1 text-green-400">
            <CreditCard size={14} />
            <span className="text-sm font-bold text-fg">12.4M</span>
          </div>
          <p className="text-[10px] text-fg4">{isFa ? "درآمد" : "Earnings"}</p>
        </div>
        <div className="bg-glass border border-bdr rounded-xl p-3 text-center">
          ...
          <p className="text-[10px] text-fg4">{isFa ? "وضعیت" : "Status"}</p>
        </div>
      </div>

      {/* Profile Card */}
      <div className="bg-bg2 border border-bdr rounded-2xl p-4 sm:p-6">
        {/* Avatar & Basic Info */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6 pb-6 border-b border-bdr">
          ...
          <div>
            <p className="text-lg font-bold text-fg">{user?.name || "راننده"}</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="pt-4 border-t border-bdr">
            <h3 className="text-sm font-semibold text-fg3 mb-4 flex items-center gap-2">
              <Car size={16} />
