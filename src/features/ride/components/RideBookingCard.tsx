...
import { Car, MapPin, Navigation, Clock, CreditCard, ChevronDown, Info } from "lucide-react";
...
export function RideBookingCard({ locale = "fa" }: { locale?: string }) {
  ...
  return (
    <div className="w-full max-w-[420px] bg-[var(--bg2)]/80 backdrop-blur-xl border border-[var(--bdr)] rounded-[32px] shadow-2xl overflow-hidden" dir={isFa ? "rtl" : "ltr"}>
      <div className="p-6 space-y-6">
            ...
            <button
              ...
              className={`p-3 rounded-2xl border transition-all flex flex-col items-center gap-1 ${
                selectedType.id === type.id 
                  ? "bg-[var(--fg)] border-[var(--fg)] text-[var(--bg)] shadow-lg scale-[1.02]" 
                  : "bg-[var(--glass)] border-[var(--bdr)] text-[var(--fg)] hover:border-[var(--bdr2)]"
              }`}
            >
        ...
        {/* Price & Payment */}
        <div className="flex items-center justify-between p-4 rounded-2xl bg-[var(--glass)] border border-[var(--bdr)]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[var(--fg)] text-[var(--bg)] flex items-center justify-center">
              <CreditCard size={18} />
            </div>
            <div>
              <p className="text-[10px] font-bold text-[var(--fg4)] uppercase">{isFa ? "روش پرداخت" : "Payment"}</p>
        ...
        <Button 
          ...
          className="py-7 rounded-2xl text-lg font-black shadow-xl shadow-[var(--fg)]/10"
        >
      ...
      {/* Footer Info */}
      <div className="px-6 py-4 bg-[var(--fg)]/5 border-t border-[var(--bdr)] flex items-center justify-between">
        <div className="flex items-center gap-2 text-[10px] font-bold text-[var(--fg4)]">
          <Info size={12} />
          <span>{isFa ? "قیمت‌ها بر اساس ترافیک زنده است" : "Prices based on live traffic"}</span>
        </div>
        <ChevronDown size={16} className="text-[var(--fg4)]" />
      </div>
    </div>
  );
}
