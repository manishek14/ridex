...
export function AdminLiveMap({ locale = "fa" }: { locale?: string }) {
  ...
  return (
    <div className="relative w-full h-[600px] rounded-[32px] overflow-hidden border border-bdr shadow-2xl bg-bg2">
      <Script
      ...
      <div className="absolute top-6 right-6 z-10 flex flex-col gap-3">
        <div className="p-4 rounded-2xl bg-bg2/80 backdrop-blur-xl border border-bdr shadow-xl min-w-[180px]">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-fg4">{isFa ? "رانندگان آنلاین" : "Online Drivers"}</span>
              <span className="text-xs font-black text-green-500">۸۵۰</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-fg4">{isFa ? "سفرهای فعال" : "Active Rides"}</span>
      ...
      <AnimatePresence>
        {selectedEntity && (
          <motion.div
            ...
            className={`absolute top-6 ${isFa ? "left-6" : "right-6"} bottom-6 w-80 z-20 p-6 rounded-[32px] bg-bg2/90 backdrop-blur-2xl border border-bdr shadow-2xl overflow-y-auto`}
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <button 
                ...
                className="p-2 rounded-xl bg-glass hover:bg-bdr transition-all"
              >
                <X size={18} />
              </button>
            </div>
            ...
            {selectedEntity.entityType === "driver" ? (
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-2xl bg-glass flex items-center justify-center text-2xl">👨‍✈️</div>
                  <div>
                    <h4 className="font-black">{selectedEntity.data.user?.name || "راننده"}</h4>
                    <p className="text-xs text-fg4">{selectedEntity.data.vehicle?.model || "خودرو"}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-2xl bg-glass border border-bdr">
                    <p className="text-[9px] font-bold text-fg4 uppercase">{isFa ? "امتیاز" : "Rating"}</p>
                    <p className="text-sm font-black text-orange-500">⭐ {selectedEntity.data.rating || 0}</p>
                  </div>
                  <div className="p-3 rounded-2xl bg-glass border border-bdr">
                    <p className="text-[9px] font-bold text-fg4 uppercase">{isFa ? "سفرها" : "Rides"}</p>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-xs font-bold">
                    <span className="text-fg4">{isFa ? "قیمت سفر" : "Price"}</span>
                    <span>{selectedEntity.data.price?.toLocaleString() || 0} {isFa ? "تومان" : "IRT"}</span>
                  </div>
                  <div className="flex justify-between text-xs font-bold">
                    <span className="text-fg4">{isFa ? "وضعیت" : "Status"}</span>
      ...
      {!isLoaded && (
        <div className="absolute inset-0 flex items-center justify-center bg-bg2/50 backdrop-blur-sm z-20">
          <div className="w-8 h-8 border-4 border-fg border-t-transparent rounded-full animate-spin" />
        </div>
      )}
    </div>
  );
