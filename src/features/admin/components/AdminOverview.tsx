    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" dir={isFa ? "rtl" : "ltr"}>
      {stats.map((stat, i) => (
        <motion.div
          ...
          className="p-6 rounded-[32px] bg-[var(--bg2)] border border-[var(--bdr)] hover:border-[var(--bdr2)] transition-all group"
        >
          <div className="flex items-center justify-between mb-4">
            <div className="w-12 h-12 rounded-2xl bg-[var(--glass)] border border-[var(--bdr)] flex items-center justify-center group-hover:bg-[var(--fg)] group-hover:text-[var(--bg)] transition-all">
              {stat.icon}
            </div>
          ...
          <p className="text-xs font-bold text-[var(--fg4)] mb-1 uppercase tracking-widest">{stat.label}</p>
          <div className="flex items-baseline gap-1">
            <h4 className="text-2xl font-black text-[var(--fg)]" style={{ fontFamily: isFa ? "Vazirmatn, sans-serif" : undefined }}>
              {stat.value}
            </h4>
