      ...
      <div className="space-y-4">
        {pendingDrivers.map((driver, i) => (
          <motion.div
            ...
            className="p-5 rounded-[24px] bg-bg2 border border-bdr hover:border-bdr2 transition-all flex flex-col md:flex-row md:items-center justify-between gap-6"
          >
            <div className="flex items-center gap-4">
              <Avatar name={driver.name} size="md" />
              <div>
                <h4 className="text-sm font-black text-fg">{driver.name}</h4>
                <p className="text-[11px] text-fg3 font-medium">{driver.car}</p>
              </div>
            </div>

            <div className="flex flex-wrap gap-2">
              {driver.docs.map((doc, j) => (
                <div key={j} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-glass border border-bdr text-[9px] font-bold text-fg4">
                  <FileText size={12} />
                  {doc}
                </div>
              ))}
            ...
            <div className="flex items-center gap-3 border-t md:border-t-0 pt-4 md:pt-0 border-bdr">
              <button className="w-10 h-10 rounded-xl bg-glass border border-bdr flex items-center justify-center text-fg4 hover:text-fg transition-all">
                <Eye size={18} />
              </button>
